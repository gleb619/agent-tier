...
- **Detached mode (default):** Child is spawned detached; stdout/stderr go to `/tmp/at-logs/at-<timestamp>-<agent>.log`. Caller gets `[at] started <agent> (pid N) — logs: <path>` and returns immediately.
- **Stream mode (`-s`):** Child inherits the terminal's stdio directly.
