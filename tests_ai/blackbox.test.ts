...
describe('blackbox agent', () => {
  ...
  it('should create test_result.txt with hello world', async () => {
    if (!agentBinExists('blackbox')) {
      console.log('SKIP: blackbox binary not found');
      return;
    }
    ...
    const exitCode = await new Promise<number>((resolve) => {
      const child = spawn('node', [cliPath, '-s', '-a', 'blackbox', '-p', prompt, '--timeout', String(DEFAULT_TIMEOUT)], {
        cwd: sandbox.dir,
        stdio: 'inherit',
      });
