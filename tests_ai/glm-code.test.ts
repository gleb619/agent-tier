...
describe('glm-code agent', () => {
  ...
  it('should create test_result.txt with hello world', async () => {
    if (!agentBinExists('glm-code')) {
      console.log('SKIP: glm-code binary not found');
      return;
    }
    ...
    const exitCode = await new Promise<number>((resolve) => {
      const child = spawn('node', [cliPath, '-s', '-a', 'glm-code', '-p', prompt, '--timeout', String(DEFAULT_TIMEOUT)], {
        cwd: sandbox.dir,
        stdio: 'inherit',
      });
